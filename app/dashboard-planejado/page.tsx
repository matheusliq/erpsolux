...
export default function DashboardPlanejadoPage() {
    ...
    const projectsWithSplit = allProjects
        ...
        .map(p => {
            const projTx = allTx.filter(t => t.projects?.id === p.id && t.type !== "Entrada");
            ...
            <div className="flex items-start justify-between mb-8">
                <div>
                    <h1 className="text-2xl font-bold tracking-tight">
                        Dashboard <span className="text-blue-500 font-light">Planejado</span>
                    </h1>
                    <p className="text-xs text-zinc-500 mt-0.5">Visão financeira e societária baseada no planejamento futuro e obras em negociação</p>
                </div>
                <button
                        ...
                        <div className="lg:col-span-2 border border-zinc-800 bg-zinc-900/40 rounded-xl p-5 overflow-hidden">
                            ...
                            {projectsWithSplit.length === 0 ? (
                                ...
                                projectsWithSplit.forEach(p => {
                                    p.splits.forEach((s: any) => { totals[s.name] = (totals[s.name] || 0) + s.value; });
                                });
                                return (
                                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                        {Object.entries(totals).map(([name, value]) => (
                                            <div key={name} className="bg-background/50 border border-zinc-800 rounded-xl p-4 text-center">
                                                ...
                                                <p className={`text-base font-extrabold mt-1 ${value >= 0 ? "text-emerald-400" : "text-rose-400"}`}>{formatBRL(value)}</p>
                                            </div>
                                        ))}
                                    </div>
                                );
                            ...
                            <div className="relative bg-background border border-zinc-800 rounded-2xl shadow-2xl max-w-2xl w-full mx-4 max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
                                <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800">
                                    <div>
                                        ...
                                        <p className="text-[10px] text-zinc-500 mt-0.5">Distribuição líquida planejada para cada sócio por obra</p>
                                    </div>
